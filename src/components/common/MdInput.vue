<template>
  <div class="md-field" :class="{'md-focused': isFocused, 'has-tips':isShowTips,  'has-value': !!value}">
    ...
    <input :type="type" :name="name" :disabled="disabled" @focus="onFocus" @blur="onBlur" class="md-input" v-bind:value="value" @input="onInput" @change="$emit('input-change', $event.target.value)" :maxlength="maxlength || ''" autocomplete="off">
    <slot></slot>
    <p class="md-tips" v-if="isShowTips">
      <span class="error" v-if="error">{{error}}</span>
      <span v-else-if="fixedTip">{{fixedTip}}</span>
      <span v-else></span>
