  ...
  <transition name="fancy">
    <div v-show="visible" class="fancy-box-mask">
      <div ref="fancyBoxWrapper" class="fancy-box-image-wrapper">
        <div v-for="(image, index) in prev" v-bind:key="index" :style="{ webkitTransform: 'translate3d(-'+ (index+1)*image.scaleWidth +'px, 0, 0)' }">
        ...
        <div v-for="(image, index) in next" v-bind:key="index" :style="{ webkitTransform: 'translate3d('+ (index+1)*image.scaleWidth +'px, 0, 0)' }">
          <div :style="{ webkitTransform: 'translate3d(0, '+ image.y +'px, 0)' }"><img :style="{ width: ''+ image.w +'px' }" :src="image.url"></div>
        </div>
      </div>
    </div>
